 ...
 */
function resolveModel(options, config) {
    if (options?.model)
 ...
 */
export async function runPlanSession(plan, config, options, agentDef, eventStream, streamContext, phaseDir) {
 ...
 */
async function processQueryStream(queryStream, eventStream, streamContext) {
    let resultMessage;
    try {
 ...
 */
function stepTypeToPhaseType(step) {
    const mapping = {
 ...
 * @returns Typed PlanResult with cost, duration, success/error status
 */
export async function runPhaseStepSession(prompt, phaseStep, config, options, eventStream, streamContext) {
