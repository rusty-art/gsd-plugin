...
function parseMustHaves(raw) {
    const defaults = { truths: [], artifacts: [], key_links: [] };
...
function normalizeArtifacts(val) {
    if (!Array.isArray(val))
        return [];
    return val
...
function normalizeKeyLinks(val) {
    if (!Array.isArray(val))
        return [];
    return val
        .filter((item) => typeof item === 'object' && item !== null)
 ...
 */
function extractElement(taskBody, tagName) {
 ...
 */
function extractTaskType(taskTag) {
 ...
 * Returns an array of file paths (lines starting with @).
 */
function extractExecutionContext(content) {
