...
- **DEV-03 (smoke test pollution):** Plan Task 4's suggested regression test string (`@~/.claude/get-shit-done/zzz-drift-test-fake.md`) literally appears in the plan document itself. Because the detector scans the active planning tree, that fake path is already in the baseline count. Appending it to `skills/gsd-next/SKILL.md` just duplicates an existing unique ref and does NOT trigger a regression. Substituted `xxx-smoke-test-unique-260421.md` (verified unique across the tree) and the smoke test then passed end-to-end as designed. Same issue applies to the plan's `fake-new-ref.md` example.
...
- **Investigation steps:**
  ...
  4. Confirmed no code changes landed in `skills/ agents/ references/ templates/` between 2026-04-21 (CONTEXT census date) and execution time. The upstream-sync commit `b4e8796` from earlier today touched `skills/gsd-sketch/SKILL.md` but did not add any `@~/.claude/get-shit-done/` refs.
...
- **What the plan said:** Append `@~/.claude/get-shit-done/zzz-drift-test-fake.md` to `skills/gsd-next/SKILL.md`, expect the detector to exit 1 with Status: FAIL.
...
- **Files affected:** None persisted — smoke test cleans up its own pollution via `git checkout -- skills/gsd-next/SKILL.md`. Final `git status` confirmed no lingering changes to `skills/`.
